...
const projectsData = [
  ...
  },
  {
    ...
    link: "#",
    github: "#",
    iframeUrl: "https://www.openstreetmap.org/export/embed.html?bbox=-0.00401%2C51.47612%2C0.00386%2C51.48007&layer=mapnik"
  },
  {
    ...
    link: "#",
    github: "#",
    iframeUrl: "https://embed.windy.com/embed2.html?lat=13.00&lon=122.00&zoom=5&level=surface&overlay=wind&product=ecmwf&menu=&message=&marker=&calendar=now&pressure=&type=map&location=coordinates&detail=&metricWind=default&metricTemp=default&radarRange=-1"
  }
];
...
function ProjectCard({ project, index, activeProjectIndex }: ProjectCardProps) {
  ...
  return (
    <ScrollStackItem
      ...
      itemClassName={`${project.bgGradient} text-white flex flex-col justify-between cursor-target overflow-hidden p-0`}
    >
      <div
        className="w-full h-full flex flex-col justify-between p-8 md:p-12 box-border relative rounded-[40px] overflow-hidden"
        onMouseEnter={() => setIsHovered(true)}
        onMouseLeave={() => setIsHovered(false)}
      >
        {/* Decorative gradient overlay */}
        <div className={`absolute inset-0 bg-gradient-to-br ${project.accentColor} pointer-events-none rounded-[40px] z-0`} />

        {/* Live Iframe Preview Background */}
        <div
          className={`absolute inset-0 transition-opacity duration-700 ease-in-out rounded-[40px] overflow-hidden ${
          ...
          {shouldLoadIframe && (
            <>
              <iframe
                ...
                className="w-full h-full border-none scale-105 origin-center filter brightness-[0.7] contrast-[1.05]"
